...
switch ($metodo) {
            ...
            case '/signin': // ------------------------------------------------------------------------------------------------------
                if(isset($_POST['email']) && isset($_POST['clave']) && isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['telefono']) && isset($_POST['tipo'])){
                    ...
                    if(!is_int($auth)){
                        ...
                        echo Response::data2DTO("Error",409, "Ya existe un usuario con el mail ".$_POST['email']);
                    } else {
    ...
    case 'GET': // ------------------------------------------------------------------------------------------------------------------
        ...
        switch ($path){
            case '/detalle': // -----------------------------------------------------------------------------------------------------
                $user = Users::getUser($decoded->sub);
                if($user != null){
                    echo Response::data2DTO("Success",200, $user);
                } else {
                    echo Response::data2DTO("Success",204, $list);
                }
                break;
            case '/lista': // -------------------------------------------------------------------------------------------------------
                ...
                if(Count($list) > 0){
                    echo Response::data2DTO("Success",200, $list);
                } else {
                    echo Response::data2DTO("Success",204, $list);
                }
                break;
